...
export default {
  ...
  methods: {
    ...
    deleteData() {
      this.$axios({
        url: "http://47.98.181.209:9090/station_delete",
        params: { StationID: this.deleteID },
      }).then((res) => {
        if (res.data.code == 1) {
          this.rawData = this.rawData.filter((e) => {
            return e.StationID != this.deleteID;
          });
    ...
    changeData() {
      if (this.formMode == "修改测站信息") {
        this.$axios({
          url: "http://47.98.181.209:9090/station_update",
        ...
        this.$axios({
          url: "http://47.98.181.209:9090/station_insert",
          params: this.changeDataForm,
        }).then((res) => {
          if (res.data.code == 1) {
            this.$message({ message: res.data.message, type: "success" });
    ...
    handleFormData() {
      this.loading = true;
      this.$axios({
        url: "http://47.98.181.209:9090/station",
      }).then((res) => {
